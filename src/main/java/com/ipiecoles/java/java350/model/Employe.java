...
public class Employe {
    ...
    public Integer getNbRtt(LocalDate d) {
        int yearDaysCount = d.isLeapYear() ? 366 : 365;
        ...
        switch (LocalDate.of(d.getYear(), 1, 1).getDayOfWeek()) {
            ...
            case FRIDAY:
                if (d.isLeapYear()) {
                    minWeekendDays = minWeekendDays + 2;
                } else {
                    minWeekendDays = minWeekendDays + 1;
                }
            case SATURDAY:
        ...
        return (int) Math.ceil((yearDaysCount - Entreprise.NB_JOURS_MAX_FORFAIT - minWeekendDays - Entreprise.NB_CONGES_BASE - monInt) * tempsPartiel);
    }
    ...
    public void augmenterSalaire(double pourcentage) {
        if (pourcentage > 0) {
            this.salaire = this.salaire * (1.0 + (pourcentage / 100));
        }
    }
